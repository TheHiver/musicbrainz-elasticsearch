...
public class MusicTrackDocumentProcessor extends EsDocumentProcessor<Track> {


    private String documentType;

    @Override protected void fillContentBuilder(XContentBuilder content, Track track)
        throws IOException {
        content.field("id", track.getId());
        content.field("gid", track.getGid());
        content.field("name", track.getTitle());
        content.field("position", track.getPosition());
        if ((track.getTags() != null) && !track.getTags().isEmpty()) {
            content.array("tags", track.getTags().toArray(new String[track.getTags().size()]));
        }
        fillReleaseBuilder(content,track.getRelease());
    }

    protected void fillReleaseBuilder(XContentBuilder content, Release release) throws IOException {
    ...
    @Override protected String getDocumentId(Track item) {
        return item.getGid();
    }

    @Override protected String getDocumentType(Track item) {
        return documentType;
    }
