...
public class MusicAlbumDocumentProcessor extends EsDocumentProcessor<Album> {
    ...
    @Override protected void fillContentBuilder(XContentBuilder content, Album album)
        throws IOException {
    ...
    private void fillArtistBuilder(XContentBuilder content, Artist artist) throws IOException {
        ...
        artistBuilder.field("country_name", artist.getArea());
        if (artist.getGender() != null) {
            artistBuilder.field("gender", artist.getGender());
        }
    ...
    @Override protected String getDocumentId(Album item) {
        return item.getGid();
    }

    @Override protected String getDocumentType(Album item) {
        return documentType;
    }
