...
@RunWith(SpringJUnit4ClassRunner.class) @ContextConfiguration @DirtiesContext
public class TestDatabaseSetup {


    @Autowired private DataSource dataSource;

    @Autowired private JdbcTemplate musicBrainzJdbcTemplate;
    ...
    @Test public void selectReleaseGroup() {
        String sql =
            "SELECT " + "    release_group.id AS albumId," + "    release_group.gid AS albumGid,"
                + "    release_group.type AS albumPrimaryTypeId,"
                + "    release_name.name AS albumName," + "    artist_name.name AS artistName,"
                + "    artist.gid AS artistGid," + "    artist.type as artistTypeId,"
                + "    artist.begin_date_year artistBeginDateYear,"
                + "    artist.gender as artistGenderId," + "    area.name as artistCountryName,"
                ...
                + "    release_group_meta.rating_count albumRatingCount " + "FROM" + "    artist"
                ...
                + " INNER JOIN release_name" + "    ON release_name.id = release_group.name"
                ...
                + "    ON release_group_meta.id = release_group.id " + "WHERE"
                + "    release_group.type = '1'" + "	  AND artist_credit.artist_count = 1"
